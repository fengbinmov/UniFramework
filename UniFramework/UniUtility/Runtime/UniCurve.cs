...
{
    public class UniCurve
    {
        public static Vector3[] GetSmoothPath(Vector3[] path, int resolution = 5,bool loop = false)
        {
            if (path == null || path.Length < 1 || resolution <= 0) return path;

            int SmoothAmount = path.Length * resolution;
            ...
            {
                waypoints[i] = new SmoothPath.Waypoint() { position = path[i] };
            }
            smoothPath.m_Looped = loop;
            smoothPath.m_Resolution = resolution;
        ...
        public static Vector3[] GetSmoothPath(Vector3[] path, float stepLength, bool loop = false,int resolution = 5)
        {
            if (path == null || path.Length < 1 || resolution <= 0) return path;
            ...
            int count = Mathf.CeilToInt(smoothPath.PathLength / Math.Abs(stepLength));

            for (int i = 0; i < count; i++)
            {
                float v = (float)i / (count - 1);
                result.Add(smoothPath.EvaluatePositionAtUnit(v, PathBase.PositionUnits.Normalized));
            }

            return result.ToArray();
        }

        public static Vector3[] Segmentation(Vector3[] path, float stepLength, bool loop = false)
        {
            if (path == null || path.Length <= 1 || stepLength <= 0) return path;

            float pathLength = 0;

            for (int i = 0; i < path.Length; i++)
            {
                pathLength += (i == 0) ? 0 : Vector3.Distance(path[i - 1], path[i]);
            }

            if(loop) pathLength += Vector3.Distance(path[^1], path[0]);

            float segLength = pathLength / Mathf.RoundToInt(pathLength / stepLength);
            ...
            float nowLength = 0;
            float nextLength = segLength;

            for (int i = 1; i < path.Length; i++)
            {
                float segmentLength = Vector3.Distance(path[i], path[i-1]);
                nowLength += segmentLength;

                while (nowLength >= nextLength)
                {
                    float p = (nowLength - nextLength) / segmentLength;
                    Vector3 p1 = Vector3.Lerp(path[i], path[i - 1], p);

                    result.Add(p1);

                    nextLength += segLength;
                }

                if (!loop && i == (path.Length - 1) && nextLength - nowLength < segLength)
                {
                    result.Add(path[i]);
                }
            }
            ...
            if (loop)
            {
                float segmentLength = Vector3.Distance(path[^1], path[0]);
                nowLength += segmentLength;

                while (nowLength >= nextLength)
                {
                    float p = (nowLength - nextLength) / segmentLength;
                    Vector3 p1 = Vector3.Lerp(path[0], path[^1], p);
        ...
        public static void Smooth(Vector3[] path, int step = 1)
        {
            step = Mathf.Clamp(step, 0, 20);

            if (path.Length < 2 || step < 0) return;
            ...
            while (step-- > 0)
            {
                for (int i = 1; i < path.Length - 1; i++)
                {
                    ...
                    Vector3 p = (p1 + p0 + p2) / 3f;
                    path[i] = Vector3.Lerp(p1, p, 0.33f);
                }
            }
        }
    }
